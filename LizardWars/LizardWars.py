...
music = pygame.mixer.Sound('./resources/music.ogg')
chara1_cry = pygame.mixer.Sound('./resources/cry1.ogg')
chara2_cry = pygame.mixer.Sound('./resources/cry2.ogg')
chara3_cry = pygame.mixer.Sound('./resources/cry3.ogg')
chara4_cry = pygame.mixer.Sound('./resources/cry4.ogg')
...
pygame.display.set_caption('Lizard Fight!')
...
class Character:
    ...
    def attackprocedure(self, user, attack, player, enemy):
        ...
        if succeed:
            ...
            if attack.stateffect != "none": #if attack involves stat changes
                if attack.target == "opp":
                    attack.apply_stateffect(opp_name, opp_atk, opp_spatk, opp_df, opp_spdf, opp_hp, user, player, enemy)
                else:
                    attack.apply_stateffect(self.name, self.atk, self.spatk, self.df, self.spdf, self.hp, user, player, enemy)
                update_statboard()

            if attack.othereffect != "none":
                if attack.target == "opp": #if attack involves status condition
                    if user == "player":
    ...
    def check_prestatus(self): # does character have a status condition that effects them at the beginning of the turn?
        if self.status != "none":
            ...
            if self.status == "paralyzed":
                hinderval = random.randrange(0, 3)
                if hinderval > 1:
                    draw_scene(playerimg, enemyimg)
    ...
    def check_poststatus(self): # does character have a status condition that effects them at the end of the turn?
        if self.status != "none":
            recoverval = random.randrange(0, 15) ## does character recover?
            if recoverval == 0:
...
class Attack:
    ...
    def apply_stateffect(self, target_name, target_atk, target_spatk, target_df, target_spdf, target_hp, user, player, enemy): # stat changes ex. atk stat increase
...
characterlist = []
chara1 = Character("Cool Guy", "Jake", "JAKE", 100, 90, 50, 70, 70, 100, 10, 99, "none", [0,1,2,3], "Hey whats up??", "Haha is that all? :)", 20, 380, 200, './resources/jake-r.png', './resources/jake-l.png', chara1_cry)
characterlist.append(chara1)
chara2 = Character("Sorcerer", "Cynthia", "CYNTHIA", 100, 50, 90, 70, 70, 190, 15, 99, "none", [0,4,5,8], "The ancient scrolls predicted our meeting.", "How disappointing.", 20, 380, 200, './resources/cynthia-r.png', './resources/cynthia-l.png', chara2_cry)
characterlist.append(chara2)
chara3 = Character("Gentlemen", "Richard", "RICHARD", 100, 70, 70, 90, 50, 170, 25, 99, "none", [0,4,10,9], "Good day!", "I dare say if you were a cup of tea you'd be a cold one.", 20, 380, 200, './resources/richard-r.png', './resources/richard-l.png', chara3_cry)
characterlist.append(chara3)
chara4 = Character("Spider", "Spider", "SPIDER", 100, 70, 70, 50, 90, 180, 20, 99, "none", [0,6,7,11], "...", "...........", 20, 380, 200, './resources/spider-r.png', './resources/spider-l.png', chara4_cry)
...
def make_all_attacks(user_name, opp_name):
    ...
    spiderbite = Attack('Spider Bite', 'special', 10, 90, 'none', 0, 'poison', 'opp', (user_name + " bit " + opp_name + "."))
    all_attacks.append(spiderbite) #7
    lightningstrike = Attack('Lightning Strike', 'special', 70, 70, 'none', 0, 'paralysis', 'opp', (user_name + " struck " + opp_name + " with a bolt of lightning."))
    all_attacks.append(lightningstrike) #8
    fortify = Attack('Fortify', 'special', 0, 100, '+def', 2, 'none', 'user', (user_name + " slipped on some stlyish body armor."))
    all_attacks.append(fortify) #9
    heartychuckle = Attack('Hearty Chuckle', 'special', 0, 100, '-def', 1, 'none', 'opp', (user_name + " chuckled in a friendly, non-threatening manner."))
    all_attacks.append(heartychuckle) #10
    feed = Attack('Feed', 'physical', 20, 100, '+hp', 2, 'none', 'user', (user_name + " nibbled on " + opp_name + "."))
    all_attacks.append(feed) #11
    return all_attacks
...
announce(player.title + " " + player.name + " is ready for battle!")
update()
player.sound.play()
wait()

draw_scene(playerimg, enemyimg)
announce(enemy.title + " " + enemy.name + " challenges " + player.name + "!")
update()
enemy.sound.play()
wait()
draw_scene(playerimg, enemyimg)
