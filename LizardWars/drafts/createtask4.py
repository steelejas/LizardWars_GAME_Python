...
class Character:
    ...
    def take_turn(self, player, enemy): # for enemy only - enemy makes choice and acts on it - self = enemy
        ...
        if enemy_choice == 0 or 1 or 2 or 3 or 4 or 5 : # attack
            ...
            attack = attacklist[self.attacks[enemy_choice]]
...
characterlist = []
chara1 = Character("      ", "Jake", "JAKE", 100, 90, 50, 70, 70, 100, 10, 99, "none", [0,1,2,3], "Hey whats up??", "Haha is that all? :)", 30, 400, 200, './resources/jake-r.png', './resources/jake-l.png', chara1_cry)
characterlist.append(chara1)
chara2 = Character("", "Cynthia", "Cynthia", 100, 50, 90, 70, 70, 190, 15, 99, "none", [0,1,4,5], "The ancient scrolls predicted our meeting.", "How disappointing.", 30, 400, 200, './resources/cynthia-r.png', './resources/cynthia-l.png', chara2_cry)
characterlist.append(chara2)
chara3 = Character("", "Richard", "RICHARD", 100, 70, 70, 90, 50, 170, 25, 99, "none", [0,1,2,4], "Good day!", "I dare say if you were a cup of tea you'd be a cold one.", 30, 400, 200, './resources/richard-r.png', './resources/richard-l.png', chara3_cry)
characterlist.append(chara3)
chara4 = Character("  ", "Spider", "SPIDER", 100, 70, 70, 50, 90, 180, 20, 99, "none", [0,2,6,7], "...", "...........", 30, 400, 200, './resources/spider-r.png', './resources/spider-l.png', chara4_cry)
...
def make_all_attacks(user_name, opp_name):
    ...
    punch = Attack("Punch", "physical", 60, 100, "none", 0, "none", "opp", (user_name + " punched " + opp_name + "."))
    all_attacks.append(punch) #1
    kick = Attack("Kick", "physical", 70, 100, "none", 0, "none", "opp", (user_name + " kicked " + opp_name + "."))
    all_attacks.append(kick) #2
    grandslam = Attack("Grand Slam", "physical", 90, 100, "none", 0, "none", "opp", (user_name + " slammed " + opp_name + " out of the park."))
    ...
    lullaby = Attack('Lullaby', 'special', 0, 100, 'none', 0, 'sleep', 'opp', (user_name + " sang a lullaby to " + opp_name + "."))
    all_attacks.append(lullaby) #5
    stickyweb = Attack('Sticky Web', 'special', 0, 100, 'none', 0, 'paralysis', 'opp', (user_name + " wrapped " + opp_name + " in a sticky web."))
    all_attacks.append(stickyweb) #6
    spiderbite = Attack('Spider Bite', 'special', 0, 100, 'none', 0, 'poison', 'opp', (user_name + " bit " + opp_name + "."))
    all_attacks.append(spiderbite) #7
    return all_attacks

attacklist = make_all_attacks(player.name, enemy.name)
...
while battling:
    ...
    if player_choice == 1: # attack
        ...
        attack1 = attacklist[player.attacks[0]]
        attack2 = attacklist[player.attacks[1]]
        attack3 = attacklist[player.attacks[2]]
        attack4 = attacklist[player.attacks[3]]

        player_choice = choose(attack1.name, attack2.name, attack3.name, attack4.name)
        attack = attacklist[player.attacks[player_choice - 1]]
